...
public class CFGFactory {
	...
	public CFG newInstance(FunctionDefBase functionDefinition) {
		try {
			...
			if (!function.getContinueStatements().isEmpty()) {
				System.err.println("warning: unresolved continue statement in function: "+functionDefinition.getEscapedCodeStr());
	...
	public static CFG newInstance(ASTNode... nodes) {
		...
		try {
			...
			// e.printStackTrace();
			System.err.println("Error in CFGFactory for nodes: "+nodes.toString());
			return newErrorInstance();
		}
	}
	...
	public static CFG newInstance(DoStatement doStatement) {
		try {
			...
			if (nVerticesOfBody == 2) {
				doBlock.addEdge(conditionContainer, conditionContainer, CFGEdge.TRUE_LABEL);
			} else {
	...
	public static CFG newInstance(TryStatement tryStatement) {
		try {
			...
			for (CatchStatement catchStatement : tryStatement.getCatchList()) {
				...
				tryCFG.mountCFG(tryCFG.getExceptionNode(), tryCFG.getExitNode(), catchBlock,
	...
	public static CFG newInstance(SwitchStatement switchStatement) {
		try {
			...
			for (CFGEdge edge : switchBody.incomingEdges(switchBody.getExitNode())) {
	...
	public static CFG newInstance(GotoStatement gotoStatement) {
		try {
			...
			gotoBlock.addGotoStatement(gotoContainer, gotoStatement.getTargetName());
			return gotoBlock;
		} catch (Exception e) {
			// e.printStackTrace();
			return newErrorInstance();
	...
	public static CFG newInstance(ContinueStatement continueStatement) {
		try {
			...
			continueBlock.addEdge(continueBlock.getEntryNode(), continueContainer);
			continueBlock.addEdge(continueContainer, continueBlock.getExitNode());
	...
	public static CFG newInstance(ThrowStatement throwStatement) {
		try {
			...
			throwBlock.addEdge(throwContainer, exceptionNode, CFGEdge.EXCEPT_LABEL);
